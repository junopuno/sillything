...
function getWidgetHeaderStyleString (style) {
  const s = { ...createDefaultStyle(), ...style }
  return `background:${s.headBg}; color:${
    s.headerTextCol
  }; font-size:${normalizeCssSize(s.headerFontSz, '16px')}; font-family:${
    s.headerFont
  }; padding:${s.headerPadding}; border-bottom:${
    s.headerBorderBottom
  }; height:${s.headerHeight}; display:${s.showHeader ? 'flex' : 'none'}`
}

function getWidgetBodyStyleString (style) {
  const s = { ...createDefaultStyle(), ...style }
  return `font-size:${normalizeCssSize(s.fontSz, '14px')}; padding:${
    s.bodyPadding
  }`
}
...
function getPlannerStats (category = null, subcategoryId = undefined) {
  ...
  if (subcategoryId !== undefined) {
    widgets = widgets.filter(
      widget => (widget.subcategoryId || null) === subcategoryId
    )
  }
  ...
  const overdue = tasks.filter(
    task =>
      task.deadline && !task.done && new Date(task.deadline) < startOfToday()
  ).length
...
function renderCategoryIcon (category, extraClass = '') {
  if (category.iconImage) {
    return `<img class="category-image-icon ${extraClass}" src="${escapeHtml(
      category.iconImage
    )}" alt="">`
  }

  return `<i class="far ${escapeHtml(
  ...
  return `
    <details class="widget-section-menu">
      ...
      <div class="widget-section-dropdown">
        <button onclick="moveWidgetToSubcategory(${widgetIndex}, '')" class="${
    !widget.subcategoryId ? 'active' : ''
  }">No section</button>
        ${(category.subcategories || [])
          .map(
            sub => `
          <button onclick="moveWidgetToSubcategory(${widgetIndex}, '${
              sub.id
            }')" class="${widget.subcategoryId === sub.id ? 'active' : ''}">
            ${escapeHtml(sub.name)}
          </button>`
          )
          .join('')}
...
function getWidgetEmbedTarget (widgetData) {
  ...
  if (widgetData.type === 'spotify') {
    const embedUrl =
      typeof getSpotifyEmbedUrl === 'function'
        ? getSpotifyEmbedUrl(widgetData.spotifyUrl || '')
        : ''
...
function hydratePreservedEmbeddedWidget (
  existingWidget,
  desiredWidget,
  widgetData
) {
  existingWidget.setAttribute(
    'class',
    desiredWidget.getAttribute('class') || 'widget'
  )
  existingWidget.setAttribute(
    'style',
    desiredWidget.getAttribute('style') || ''
  )
  ...
  if (desiredHeader && existingHeader)
    existingHeader.innerHTML = desiredHeader.innerHTML
  ...
  if (desiredBody && existingBody) {
    ...
    if (input) {
      if (widgetData?.type === 'spotify')
        input.value = widgetData.spotifyUrl || ''
      else input.value = widgetData.youtubeUrl || widgetData.embedUrl || ''
    }

    const desiredFrame = desiredBody.querySelector(
      '.youtube-frame, .spotify-frame'
    )
    const existingFrame = existingBody.querySelector(
      '.youtube-frame, .spotify-frame'
    )
    if (existingFrame && desiredFrame) {
      ...
      if (
        desiredFrameSrc &&
        existingFrame.getAttribute('src') !== desiredFrameSrc
      ) {
    ...
    const desiredFallbackLink = desiredBody.querySelector(
...
function renderTodayPanel () {
  ...
  const todaysTasks = getAllTasks()
    .filter(
      task => !task.done && task.deadline && new Date(task.deadline) < tomorrow
    )
    .slice(0, 6)
        ...
        ${
          todaysTasks.length
            ? todaysTasks
                .map(
                  task => `
          <button class="today-task" onclick="openProject(${
            task.categoryIndex
          })">
            <span>${escapeHtml(task.text)}</span>
            <small>${escapeHtml(task.categoryName)} · ${escapeHtml(
                    task.priority
                  )}</small>
          </button>`
                )
                .join('')
            : '<p>No dated tasks for today. Nice and clean.</p>'
        }
      </div>
    </section>`
}
...
function renderSubcategoryTabs (category) {
  ...
  return `
    <div class="subcategory-tabs">
      <button class="${
        activeSubId === null ? 'active' : ''
      }" onclick="selectSubcategory(null)">All</button>
      <button class="${
        activeSubId === 'uncategorized' ? 'active' : ''
      }" onclick="selectSubcategory('uncategorized')">No section</button>
      ${subcategories
        .map(
          sub => `
        <button class="${
          activeSubId === sub.id ? 'active' : ''
    ...
    <div class="subcat-manager">
      ...
      ${
        ...
        <div class="subcat-row">
          ...
          <button onclick="selectSubcategory('${
            sub.id
          }')" title="Open"><i class="fas fa-arrow-right"></i></button>
          <button class="danger-icon" onclick="deleteSubcategory('${
            sub.id
...
function render () {
  ...
  if (activeIdx === null) {
    ...
    html += frontPageWidgets
      .map(
        w => `
      <div class="widget" data-geo-id="${w.id}" data-x="${w.pos.x}" data-y="${
          w.pos.y
        }" 
           style="transform: translate(${w.pos.x}px, ${w.pos.y}px); width:${
          w.size.w
        }px; height:${w.size.h}px; --widget-scale:${getWidgetScale(
          w
        )}; ${getWidgetStyleString(w.style)}">
          <div class="widget-header" style="${getWidgetHeaderStyleString(
            w.style
          )}">
              <input type="text" value="${escapeHtml(
                w.title
              )}" onchange="updateFrontTitle('${w.id}', this.value)">
              <div class="widget-controls-group"><i class="fas fa-ellipsis-v" onclick="openInspector('front','${
                w.id
              }')"></i></div>
          </div>
          <div id="inspect-front-${
            w.id
          }" class="design-inspector hidden">${renderInspectorMarkup(
          'front',
          w.id,
          w.style
        )}</div>
          <div class="widget-body" style="${getWidgetBodyStyleString(w.style)}">
              ${
                w.type === 'date'
                  ? parseHtmlDateBlock()
                  : parseHtmlCalendarBlock(w.style?.textCol)
              }
          </div>
      </div>`
      )
    ...
    html += data
      .map((p, i) => {
        ...
        return `<div class="category-shortcut-card" data-cat-index="${i}" data-x="${x}" data-y="${y}" onclick="openProject(${i}, event)" style="position:absolute; transform: translate(${x}px, ${y}px); width:${
          p.size?.w ?? 220
        }px; height:${p.size?.h ?? 140}px; border-color:${
          p.accent || '#e2e8f0'
        };">
                <div>${renderCategoryIcon(p)}<h4>${escapeHtml(
          p.name
        )}</h4></div>
                <span>${p.widgets.length} widgets · ${
    ...
    const stats = getPlannerStats(
      category,
      activeSubId === null ? undefined : selectedSubId
    )
    const rewardState = category.rewardState || {
      completedTasks: 0,
      streak: 0,
      badges: []
    }
    ...
    if (rewardBanner) {
      rewardBanner.innerHTML = `
        ...
        <span>${
          (rewardState.badges || []).length
            ? `🏅 ${rewardState.badges.join(' · ')}`
            : '🏷️ no badges yet'
        }</span>`
    }
    ...
    if (canvasControls)
      ...
      <select id="cat-bg-type" onchange="updateCategoryBgType(this.value, false)">
        <option value="solid" ${
          category.bgType === 'solid' ? 'selected' : ''
        }>Solid color</option>
        <option value="gradient" ${
          category.bgType === 'gradient' ? 'selected' : ''
        }>Gradient</option>
      </select>
      <label>Canvas color</label>
      <input type="color" id="cat-bg-picker" value="${
        category.bgColor || '#f8fafc'
      }" oninput="updateCategoryBg(this.value, false)">
      <label>Gradient start</label>
      <input type="color" id="cat-bg-gradient-start" value="${
        category.bgGradientStart || '#ffffff'
      }" oninput="updateCategoryBgGradient(this.value, document.getElementById('cat-bg-gradient-end').value, false)">
      <label>Gradient end</label>
      <input type="color" id="cat-bg-gradient-end" value="${
        category.bgGradientEnd || '#e0f2fe'
      }" oninput="updateCategoryBgGradient(document.getElementById('cat-bg-gradient-start').value, this.value, false)">
      <label>Angle</label>
      <input type="range" min="0" max="360" step="5" value="${
        category.bgGradientAngle || 135
      }" oninput="updateCategoryBgAngle(this.value, false)">
      <label>Accent</label>
      <input type="color" value="${
        category.accent || '#2563eb'
      }" oninput="updateCategoryAccent(this.value)">
    ...
    if (categoryControls)
      ...
      <input type="text" value="${escapeHtml(
        category.name
      )}" oninput="updateCategoryName(this.value, false)" onchange="updateCategoryName(this.value)">
      <label>Icon</label>
      <select onchange="updateCategoryIcon(this.value)">
        ${[
          ...
          .map(
            icon =>
              `<option value="${icon}" ${
                category.icon === icon ? 'selected' : ''
              }>${icon.replace('fa-', '')}</option>`
          )
          .join('')}
      </select>
      <label>Image icon</label>
      <input type="url" value="${escapeHtml(
        category.iconImage || ''
      )}" placeholder="Paste image URL" onchange="updateCategoryIconImage(this.value)">
    ...
    const visibleWidgets = category.widgets
      ...
      .filter(
        item =>
          activeSubId === null ||
          (activeSubId === 'uncategorized'
            ? !item.widget.subcategoryId
            : item.widget.subcategoryId === activeSubId)
      )

    const categoryHtml =
      renderSubcategoryTabs(category) +
      visibleWidgets
        .map(({ widget: w, index: i }) => {
          // ==========================================
          ...
          if (w.type === 'calculator' || w.type === 'graph') {
            ...
            const widgetStyle = `transform: translate(${w.pos.x}px, ${
              w.pos.y
            }px); width:${w.size.w || 360}px; height:${
              w.size.h || 430
            }px; resize: both; overflow: hidden; min-width: 300px; min-height: 350px;`

            return `
        <div class="widget y2k-calc-override" data-index="${i}" data-x="${
              w.pos.x
            }" data-y="${w.pos.y}" style="${widgetStyle}">
          ...
          if (w.type === 'mp3') {
            const widgetStyle = `transform: translate(${w.pos.x}px, ${
              w.pos.y
            }px); width:${w.size.w || 340}px; height:${
              w.size.h || 320
            }px; resize: both; overflow: hidden; min-width: 260px; min-height: 240px; --widget-scale:${getWidgetScale(
              w
            )}; background:${w.style.bodyBg}; color:${
              w.style.textCol
            }; border-color:${w.style.borderCol}; border-width:${
              w.style.borderWidth
            }; border-radius:${w.style.cornerRadius};`

            return `
        <div class="widget" data-index="${i}" data-x="${w.pos.x}" data-y="${
              w.pos.y
            }" style="${widgetStyle}">
            <div class="widget-header" style="${getWidgetHeaderStyleString(
              w.style
                ...
                <input type="text" value="${escapeHtml(
                  w.title
                )}" onchange="updateWidgetProp(${i}, 'title', this.value)">
            ...
            <div id="inspect-cat-${i}" class="design-inspector hidden">${renderInspectorMarkup(
              'cat',
              i,
              w.style
            )}</div>
            <div class="widget-body" style="${getWidgetBodyStyleString(
              w.style
            )}">
          ...
          const widgetClass =
            w.type === 'image' ? 'widget image-widget' : 'widget'
          ...
          const borderColor =
            isImageWidget && hasImage ? 'transparent' : w.style.borderCol
          const borderWidth =
            isImageWidget && hasImage ? '0px' : w.style.borderWidth
          const headerHtml =
            w.type === 'image'
              ? ''
              : `
          ...
          const widgetStyle = `transform: translate(${w.pos.x}px, ${
            w.pos.y
          }px); width:${w.size.w}px; height:${
            w.size.h
          }px; --widget-scale:${getWidgetScale(
            w
          )}; background:${bgColor}; color:${
            w.style.textCol
          }; border-color:${borderColor}; border-width:${borderWidth}; border-radius:${
            w.style.cornerRadius
          }`

          return `
        <div class="${widgetClass}" data-index="${i}" data-x="${
            w.pos.x
          }" data-y="${w.pos.y}" 
             style="${widgetStyle}">
            ${headerHtml}
  ...
  if (openInspectorState) {
    const openEl = document.getElementById(
      `inspect-${openInspectorState.scope}-${openInspectorState.ref}`
    )
