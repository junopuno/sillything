...
let plannerEvents = JSON.parse(localStorage.getItem('devos_planner_events_v2')) || {};

// Ändrad till ett objekt där nyckeln blir veckan, t.ex. "2026-W26"
let plannerWeeklyData = JSON.parse(localStorage.getItem('devos_planner_weekly_data')) || {};
...
function saveWeeklyDataState() {
  localStorage.setItem('devos_planner_weekly_data', JSON.stringify(plannerWeeklyData));
}
...
function savePlannerEvent() {
  ...
  localStorage.setItem('devos_planner_events_v2', JSON.stringify(plannerEvents));
  closeEventModal();
  renderFullscreenPlanner();
}
